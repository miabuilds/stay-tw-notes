#!/usr/bin/env python3
# life.js / kanji-gap.js の逐字読音を萌典(教育部)の異読表と照合。base(声母韻母)不一致=要修正、声調のみ差=要確認。
...
def moe(ch):
    ...
    return cache[ch]
NODE=r'''const L=require("./life.js").LIFE,K=require("./kanji-gap.js").KANJI_GAP;const o=[];
for(const s of L.scenes)for(const l of s.lines)o.push([l.z,l.py,"life:"+s.id]);
for(const k of K){o.push([k.tw,k.py,"kanji"]);if(k.ex&&k.ex.z)o.push([k.ex.z,k.ex.py,"kanji-ex"]);}
