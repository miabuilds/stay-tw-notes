...
const YTS = (() => {
  ...
  const SAMPLES = [
    ...
    // 書き起こし式:中国語字幕が無いので時間軸だけ用意してある。聞いて自分で埋める上級者向け。
    { v: "wjGfXbmd3Lk", cat: "fill", t: "許光漢・林柏宏 默契大考驗", tag: "6 分 · VOGUE Taiwan", fill: true },
    // 講演:長めだが中身がある
  ...
  const CATS = ["all", "beg", "life", "news", "talk", "fill"];
    ...
    $("ytsBody").innerHTML = `
      ...
      <div class="yts-grid">${list.map((s) => {
        ...
        return `<button class="yts-card" onclick="YTS.load('${s.v}')">
          ...
          ${s.fill ? `<span class="yts-badge">${esc(T("ytsFillBadge"))}</span>` : ""}
